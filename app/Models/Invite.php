    ...
    protected static function booted()
    {
        ...
        static::saved(queueable(function (Invite $invite) {
            ...
            try {
                ...
                $text = '🎟️ Update for "' . $event->name . '":'. PHP_EOL . PHP_EOL;
                $text .= ($invite->is_attending ? '✅ ' : '❌ ') . $invite->name . ($invite->is_attending ? ' can attend!' : ' cannot attend.') . PHP_EOL . PHP_EOL;
                if (!empty($invite->comments)) {
                    $text .= '💬 Comment: ' . $invite->comments . PHP_EOL . PHP_EOL;
                }
                $text .= 'See: ' . url('admin/events/' . $invite->event_id);
